...
# Utilizamos la biblioteca moderna «pypdf» (successor de PyPDF2)
# Asegúrate de añadir a requirements.txt:  pypdf>=3.16.0
...
st.title(":page_facing_up: Image ⇢ PDF  •  PDF Merger")

# ────────────────────────────────────────────────────────────────────────────────
# BOTÓN UNIVERSAL LIMPIAR
# ────────────────────────────────────────────────────────────────────────────────

def clear_state():
    st.session_state.clear()

st.sidebar.button("🪑 Limpiar", on_click=clear_state)
...
def fit_image_to_letter(img):
    """Devuelve la imagen centrada en un lienzo carta sin recortes."""
...
if mode == "Convertir imágenes a PDF":
    st.header(":framed_picture:  →  :page_facing_up: Convertir imágenes a PDF (tamaño carta)")
    ...
    if st.button("Crear PDF") and imgs:
        ...
        for uf in imgs:
            try:
                ...
                pages.append(fit_image_to_letter(pil_img))
...
# ────────────────────────────────────────────────────────────────────────────────
# MODO 2 — UNIR PDFs  (estandariza a carta)
# ────────────────────────────────────────────────────────────────────────────────
else:
    st.header(":books:  →  :page_facing_up: Unir PDFs (uniforme carta)")
